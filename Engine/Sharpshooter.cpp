...
void Sharprshooter::calculatePossibleShot(bool aimsRight, bool aimsLeft, bool aimsDown, bool aimsUp, const Vec2D& pos) //pos of player
{
	if (aimsDown) {
...
bool  Sharprshooter::Shoot(bool aimsRight, bool aimsLeft, bool aimsDown, bool aimsUp, std::vector<std::unique_ptr<Enemy>>& enemy, std::vector<Obstacle>obstacles, const Vec2D& pos, float dt, bool isMoving)
{
	currentTime += dt;
...
void  Sharprshooter::CleanVector() {
	while (!possibleShot.empty())
		possibleShot.pop_back();
}

bool  Sharprshooter::CheckIfEnemyCanBeHitted(std::unique_ptr<Enemy>& enemy)
{
...
bool  Sharprshooter::DrawShot(Graphics& gfx, float dt)
{
...
void Sharprshooter::RemoveEnemiesFromVector(std::vector<std::unique_ptr<Enemy>>& enemies)
{
	enemies.clear();
}
bool  Sharprshooter::CheckIfObstacleCanBeHitted(Obstacle obstacle)
{
...
void  Sharprshooter::MakeCalculationoOfShot(const Object& obj, bool aimsRight, bool aimsLeft, bool aimsDown, bool aimsUp)
{
...
void Sharprshooter::MergeVector(std::vector<std::unique_ptr<Enemy>>& avaliableTarget, std::vector<std::unique_ptr<Enemy>>& enemy)
{
...
void Sharprshooter::AddAmo(int amo)
{
	ammo += amo;
}
