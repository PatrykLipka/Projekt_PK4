...
Font::Font(const std::string& filename,unsigned int width, unsigned int height, Color chroma)
	:
...
void Font::DrawTexts(const std::string& text, const Vec2D& pos, Color color, Graphics& gfx) const
{
...
Rect Font::MapGlyphRect(char c) const
{
	...
	return Rect(
		{ float(xGlyph * glyphWidth),float(yGlyph * glyphHeight) },
